...
# Res Block: x + conv(relu(conv(x)))
# --------------------------------------------
class ZRZResBlock(nn.Module):
    def __init__(self, in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1, bias=True, mode='CRC', negative_slope=0.2):
        super(ZRZResBlock, self).__init__()
        ...
        self.first_res = nn.Sequential(
            ...
            nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=3, bias=True, dilation=3),
        )
...
# zrzres Block: x + f(x)
# --------------------------------------------
class ZRZResBlock1(nn.Module):
    def __init__(self, in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1, bias=True, mode='CRC', negative_slope=0.2):
        super(ZRZResBlock1, self).__init__()

        assert in_channels == out_channels, 'Only support in_channels==out_channels.'
        if mode[0] in ['R', 'L']:
            mode = mode[0].lower() + mode[1:]
        self.first_res = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=64, kernel_size=3, stride=1, padding=1, bias=True, dilation=1),
            nn.ReLU(inplace=True),
        )
        self.conv_res = nn.Sequential(
            nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=3, bias=True, dilation=3),
...
class ZRZResBlock2(nn.Module):
    def __init__(self, in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1, bias=True, mode='CRC', negative_slope=0.2):
        super(ZRZResBlock2, self).__init__()

        assert in_channels == out_channels, 'Only support in_channels==out_channels.'
        if mode[0] in ['R', 'L']:
            mode = mode[0].lower() + mode[1:]
        self.first_res = nn.Sequential(
            nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1, bias=True, dilation=1),
            nn.BatchNorm2d(in_channels),
            nn.ReLU(inplace=True),
        )
        self.conv_res = nn.Sequential(
